...
ServerEngine::ServerEngine(std::string host, uint16_t port, std::size_t thread_count)
    : host_(std::move(host)),
      port_(port),
...
void ServerEngine::start() {
    ...
    spdlog::info("Starting PaperMC++ Server Engine on {}:{} with {} network worker threads...",
                 host_, port_, thread_count_);
...
void ServerEngine::do_accept() {
    acceptor_.async_accept(
        [this](asio::error_code ec, asio::ip::tcp::socket socket) {
            if (!ec) {
                std::make_shared<ServerConnection>(std::move(socket))->start();
            } else {
