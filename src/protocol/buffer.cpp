...
std::expected<int64_t, std::string_view> ByteBuf::read_i64() noexcept {
    ...
    uint32_t high_net, low_net;
    std::memcpy(&high_net, buffer_.data() + read_cursor_, 4);
    std::memcpy(&low_net, buffer_.data() + read_cursor_ + 4, 4);
    read_cursor_ += 8;

    uint64_t high = ntohl(high_net);
    uint64_t low = ntohl(low_net);
    uint64_t host_val = (high << 32) | low;
...
void ByteBuf::write_i64(int64_t value) {
    ...
    std::memcpy(bytes.data(), &high, 4);
    std::memcpy(bytes.data() + 4, &low, 4);
