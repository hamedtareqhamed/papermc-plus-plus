...
struct HandshakePacket {
  ...
  static std::expected<HandshakePacket, std::string_view>
  deserialize(ByteBuf &buf) noexcept {
    HandshakePacket pkt;
...
struct StatusRequestPacket {
  static std::expected<StatusRequestPacket, std::string_view>
  deserialize(ByteBuf & /*buf*/) noexcept {
...
struct PingPacket {
  ...
  static std::expected<PingPacket, std::string_view>
  deserialize(ByteBuf &buf) noexcept {
    auto p = buf.read_i64();
    if (!p)
...
struct LoginStartPacket {
  ...
  static std::expected<LoginStartPacket, std::string_view>
  deserialize(ByteBuf &buf) noexcept {
...
struct LoginSuccessPacket {
  ...
  void serialize(ByteBuf &buf) const {
    ...
    buf.write_bytes(std::span<const std::byte>(
        reinterpret_cast<const std::byte *>(uuid.data()), 16));
...
// Select Known Packs Packet (State CONFIGURATION, Clientbound ID 0x0E for 26.2
// / Protocol 776)
...
inline void write_nbt_string(ByteBuf &buf, std::string_view name,
...
inline void write_nbt_double(ByteBuf &buf, std::string_view name,
                             double value) {
...
inline void write_overworld_dimension_nbt(ByteBuf &buf) {
  ...
  write_nbt_byte(buf, "has_ender_dragon_fight", 0);
  ...
  // --- 2. monster_spawn_light_level Compound ---
  buf.write_u8(0x0A); // TAG_Compound
  std::string_view light_level_name = "monster_spawn_light_level";
  buf.write_u16(static_cast<uint16_t>(light_level_name.size()));
  buf.write_bytes(std::span<const std::byte>(
      reinterpret_cast<const std::byte *>(light_level_name.data()),
  ...
  buf.write_u8(0x00); // Root TAG_End
}

// Registry Data Packet (State CONFIGURATION, Clientbound ID 0x07 for 26.2 /
// Protocol 776)
...
// Update Enabled Features Packet (State CONFIGURATION, Clientbound ID 0x0C
// for 26.2 / Protocol 776)
...
// Update Tags Packet (State CONFIGURATION, Clientbound ID 0x0D for 26.2 /
// Protocol 776)
...
// Finish Configuration Packet (State CONFIGURATION, Clientbound ID 0x03
// for 26.2 / Protocol 776)
