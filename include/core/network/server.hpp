...
class ServerEngine {
public:
    ServerEngine(std::string host, uint16_t port, std::size_t thread_count = 4, bool offline_mode = true);
    ~ServerEngine();

    void start();
    void stop();

    [[nodiscard]] bool offline_mode() const noexcept { return offline_mode_; }
